        ...
        {deals.map((deal, i) => (
          ...
          >
            ...
            <div className="flex items-start justify-between">
              <div
                className={`flex items-center gap-2 text-[12px] font-dm font-semibold px-3 py-1.5 rounded-full ${
                  deal.dark
              ...
              <div
                className={`font-syne font-extrabold text-3xl ${deal.dark ? "text-[#A8E040]" : "text-[#1A4526]"}`}
              >
                {deal.discount}
              </div>
            </div>
            ...
            {/* Content */}
            <div>
              <h3 className="font-syne font-bold text-[22px] md:text-[26px] mb-2 leading-tight">
                {deal.title}
              </h3>
              <p
                className={`font-dm text-[14px] leading-relaxed ${deal.dark ? "text-white/70" : "text-gray-500"}`}
              >
                {deal.subtitle}
              </p>
            </div>
            ...
            <div className="flex items-end justify-between">
              <div>
                <div className="flex items-baseline gap-2">
                  <span className="font-syne font-extrabold text-[28px]">
                    {deal.price}
                  </span>
                  <span
                    className={`font-dm text-[13px] line-through ${deal.dark ? "text-white/40" : "text-gray-400"}`}
                  >
                    {deal.originalPrice}
                  </span>
                </div>
                <div
                  className={`font-dm text-[11px] mt-1 ${deal.dark ? "text-white/50" : "text-gray-400"}`}
                >
                  Expires in: <Countdown totalHours={deal.expiresHours} />
                </div>
              </div>
              <button
                className={`rounded-full px-6 py-3 font-dm font-bold text-[13px] inline-flex items-center gap-2 group transition-colors ${
                  deal.dark
